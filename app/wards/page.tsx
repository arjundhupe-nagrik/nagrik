...
export default function WardsPage() {
    ...
    async function load() {
      ...
      if (data) {
        ...
        data.forEach((c:any) => {
          ...
          if (!map[c.ward_number]) map[c.ward_number] = { ward:c.ward_number, total:0, open:0, resolved:0, progress:0, categories:{} }
  ...
  let filtered = [...wards]
  if (search) filtered = filtered.filter(w => w.ward.toString().includes(search))
  ...
  return (
    <main style={{ minHeight:'100vh', background:'#0D0D14', fontFamily:"'Inter',system-ui,sans-serif" }}>
      ...
      <div style={{ padding:'40px 32px 32px', maxWidth:'760px', margin:'0 auto' }}>
        <h1 style={{ fontSize:'32px', fontWeight:'900', color:'white', letterSpacing:'-1px', marginBottom:'6px' }}>Ward heatmap</h1>
        <p style={{ fontSize:'14px', color:'rgba(255,255,255,0.35)' }}>Every ward in Nagpur ranked by civic issues.</p>
      </div>

      {/* Summary */}
      <div style={{ maxWidth:'760px', margin:'0 auto', padding:'0 32px 28px', display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'10px' }}>
        {[
      ...
      <div style={{ maxWidth:'760px', margin:'0 auto', padding:'0 32px 48px' }}>
        ...
        <div style={{ display:'flex', gap:'10px', marginBottom:'20px', flexWrap:'wrap', alignItems:'center' }}>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search ward..."
            style={{ padding:'9px 14px', borderRadius:'10px', border:'1px solid rgba(255,255,255,0.08)', background:'rgba(255,255,255,0.05)', color:'white', fontSize:'13px', outline:'none', width:'160px' }} />
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(210px,1fr))', gap:'10px' }}>
          {filtered.map(w => {
            ...
            return (
              <div key={w.ward}
                style={{ background:'rgba(255,255,255,0.03)', border:`1px solid rgba(255,255,255,0.07)`, borderLeft:`3px solid ${h.color}`, borderRadius:'12px', padding:'16px', transition:'border-color 0.15s', cursor:'pointer' }}
                onMouseEnter={e => (e.currentTarget.style.borderColor='rgba(255,255,255,0.15)')}
                onMouseLeave={e => (e.currentTarget.style.borderColor='rgba(255,255,255,0.07)')}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'12px' }}>
                  <div>
                    <div style={{ fontSize:'17px', fontWeight:'800', color:'white' }}>Ward {w.ward}</div>
                    <div style={{ fontSize:'11px', color:h.color, fontWeight:'600', textTransform:'uppercase', letterSpacing:'0.3px', marginTop:'2px' }}>{h.label}</div>
                  </div>
