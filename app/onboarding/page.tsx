...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      ...
      supabase.from('profiles').select('preferred_area').eq('id', data.user.id).single().then(({ data: p }) => {
        if (p?.preferred_area) { router.push('/'); return }
        setUser(data.user)
        setChecking(false)
      })
    })
    ...
    <main style={{ minHeight:'100vh', background:'#0D0D14', fontFamily:"'Inter',system-ui,sans-serif", display:'flex', alignItems:'center', justifyContent:'center', padding:'24px' }}>
      <div style={{ width:'100%', maxWidth:'480px' }}>
        ...
        <div style={{ textAlign:'center', marginBottom:'8px' }}>
          <div style={{ fontSize:'22px', fontWeight:'900', color:'#E8731A', marginBottom:'4px' }}>nagrik</div>
        </div>

        <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'8px', marginBottom:'28px' }}>
          {[1,2].map(s => (
            <div key={s} style={{ display:'flex', alignItems:'center', gap:'8px' }}>
              <div style={{ width:'28px', height:'28px', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'12px', fontWeight:'700', background: step>=s ? '#E8731A' : 'rgba(255,255,255,0.08)', color: step>=s ? 'white' : 'rgba(255,255,255,0.3)' }}>
                {s}
              </div>
              {s < 2 && (
                <div style={{ width:'40px', height:'2px', background: step>s ? '#E8731A' : 'rgba(255,255,255,0.08)', borderRadius:'1px' }}/>
              )}
            </div>
          ))}
        </div>
            ...
            <>
              <div style={{ marginBottom:'24px' }}>
                ...
                <p style={{ fontSize:'13px', color:'rgba(255,255,255,0.35)', margin:0 }}>
                  We will show you complaints from your area first.
                </p>
              </div>
              ...
              <input
                value={areaSearch}
                onChange={e => { setAreaSearch(e.target.value); setSelectedArea(null) }}
                placeholder="Search area — Dharampeth, Sadar..."
                style={{ background:'rgba(255,255,255,0.05)', border:'1px solid '+(selectedArea ? 'rgba(232,115,26,0.4)' : 'rgba(255,255,255,0.08)'), color:'white', borderRadius:'10px', padding:'12px 14px', fontSize:'14px', width:'100%', outline:'none', marginBottom:'8px' }}
              />

              <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'10px', maxHeight:'220px', overflowY:'auto', marginBottom:'20px' }}>
                {areas.length === 0 && (
                  <div style={{ padding:'14px 16px', fontSize:'13px', color:'rgba(255,255,255,0.3)' }}>Loading areas...</div>
                )}
                ...
                {filteredAreas.slice(0,25).map(area => (
                  <div
                    ...
                    style={{ padding:'10px 14px', cursor:'pointer', display:'flex', justifyContent:'space-between', alignItems:'center', borderBottom:'1px solid rgba(255,255,255,0.04)', background: selectedArea?.ward_number===area.ward_number ? 'rgba(232,115,26,0.1)' : 'transparent', transition:'background 0.1s' }}
                    ...
                    <span style={{ fontSize:'14px', color: selectedArea?.ward_number===area.ward_number ? '#E8731A' : 'rgba(255,255,255,0.8)' }}>
                      {area.area_name}
                    </span>
              ...
              {selectedArea && (
                <div style={{ background:'rgba(61,170,110,0.08)', border:'1px solid rgba(61,170,110,0.2)', borderRadius:'8px', padding:'10px 14px', marginBottom:'16px', fontSize:'13px', color:'#3DAA6E' }}>
                  Selected: <strong>{selectedArea.area_name}</strong> — Ward {selectedArea.ward_number}
                </div>
              )}

              <button
                onClick={() => selectedArea && setStep(2)}
          ...
          {step === 2 && (
            <>
              <div style={{ marginBottom:'24px' }}>
                ...
                <p style={{ fontSize:'13px', color:'rgba(255,255,255,0.35)', margin:0 }}>
                  Add your phone number for complaint updates. Optional.
                </p>
              </div>
              ...
              <div style={{ background:'rgba(232,115,26,0.08)', border:'1px solid rgba(232,115,26,0.2)', borderRadius:'10px', padding:'14px', marginBottom:'20px', display:'flex', alignItems:'center', gap:'10px' }}>
                ...
                <button
                  onClick={() => setStep(1)}
                  style={{ marginLeft:'auto', background:'transparent', border:'none', color:'rgba(255,255,255,0.3)', cursor:'pointer', fontSize:'12px' }}>
                  change
                </button>
              </div>
