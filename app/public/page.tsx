...
export default function PublicFeed() {
  ...
  return (
    <main style={{ minHeight:'100vh', background:'#0D0D14', fontFamily:"'Inter',system-ui,sans-serif" }}>
      ...
      {/* Stats banner */}
      <div style={{ background:'rgba(232,115,26,0.08)', borderBottom:'1px solid rgba(232,115,26,0.15)', padding:'10px 20px', display:'flex', alignItems:'center', justifyContent:'center', gap:'24px' }}>
        ...
        <button onClick={() => router.push('/login')} style={{ background:'transparent', border:'none', color:'#E8731A', fontSize:'12px', fontWeight:'700', cursor:'pointer' }}>
      ...
      <div style={{ maxWidth:'520px', margin:'0 auto', padding:'20px 16px' }}>
        ...
        {loading && (
          <div style={{ textAlign:'center', padding:'48px', color:'rgba(255,255,255,0.2)' }}>Loading issues...</div>
        )}

        {!loading && complaints.length === 0 && (
          <div style={{ textAlign:'center', padding:'48px' }}>
            <div style={{ fontSize:'32px', marginBottom:'12px' }}>🏙️</div>
            <div style={{ fontSize:'15px', color:'rgba(255,255,255,0.4)' }}>No complaints yet. Be the first!</div>
            <button onClick={() => router.push('/file')} style={{ background:'#E8731A', color:'white', border:'none', padding:'12px 24px', borderRadius:'8px', fontSize:'14px', fontWeight:'700', cursor:'pointer', marginTop:'16px' }}>
              File a complaint
            </button>
          </div>
        )}
        ...
        {complaints.map(c => (
          <div key={c.id} style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'16px', marginBottom:'12px', overflow:'hidden', transition:'border-color 0.15s' }}
            ...
            <div style={{ padding:'16px 18px 12px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
                <div style={{ width:'40px', height:'40px', borderRadius:'12px', background:`${CAT_COLOR[c.category]}20`, border:`1px solid ${CAT_COLOR[c.category]}40`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'20px' }}>
              ...
              <span style={{ fontSize:'10px', fontWeight:'700', padding:'4px 10px', borderRadius:'20px',
            ...
            <div style={{ padding:'10px 18px 14px', borderTop:'1px solid rgba(255,255,255,0.05)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              ...
              <button onClick={() => router.push('/login')}
                style={{ background:'transparent', border:'none', color:'rgba(255,255,255,0.25)', cursor:'pointer', fontSize:'12px', display:'flex', alignItems:'center', gap:'4px' }}>
                🔐 Sign in to track
              </button>
            </div>
          </div>
        ...
        {!loading && complaints.length > 0 && (
          <div style={{ background:'rgba(232,115,26,0.06)', border:'1px solid rgba(232,115,26,0.2)', borderRadius:'14px', padding:'20px', textAlign:'center', marginTop:'8px' }}>
            ...
            <div style={{ fontSize:'14px', fontWeight:'700', color:'white', marginBottom:'4px' }}>
              Want to track your complaints?
            </div>
            <div style={{ fontSize:'12px', color:'rgba(255,255,255,0.4)', marginBottom:'14px', lineHeight:'1.5' }}>
              Sign in to see ward heatmaps, complaint details, and get notified when your issues are resolved.
            </div>
            <button onClick={() => router.push('/login')}
