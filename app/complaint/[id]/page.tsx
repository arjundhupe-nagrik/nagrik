...
export default function ComplaintPage() {
  ...
  if (loading) return (
    <main style={{ minHeight:'100vh', background:'#0D0D14', fontFamily:"'Inter',system-ui,sans-serif", display:'flex', alignItems:'center', justifyContent:'center' }}>
      <div style={{ color:'rgba(255,255,255,0.2)', fontSize:'14px' }}>Loading...</div>
    </main>
  )

  if (!complaint) return (
    <main style={{ minHeight:'100vh', background:'#0D0D14', fontFamily:"'Inter',system-ui,sans-serif", display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:'12px' }}>
      ...
      <div style={{ maxWidth:'620px', margin:'0 auto', padding:'40px 24px' }}>
        ...
        <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'16px', padding:'24px', marginBottom:'12px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'16px', gap:'10px' }}>
            <span style={{ fontSize:'11px', fontWeight:'700', color: CAT_COLOR[complaint.category]||'#888', textTransform:'uppercase', letterSpacing:'0.5px' }}>
              {complaint.category}
            </span>
          ...
          <h1 style={{ fontSize:'22px', fontWeight:'800', color:'white', marginBottom:'14px', lineHeight:'1.3', letterSpacing:'-0.3px' }}>
            {complaint.title}
          </h1>

          {complaint.description && (
            <p style={{ fontSize:'15px', color:'rgba(255,255,255,0.5)', lineHeight:'1.7', marginBottom:'18px' }}>
              {complaint.description}
            </p>
          )}
          ...
          <div style={{ borderTop:'1px solid rgba(255,255,255,0.06)', paddingTop:'16px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'14px' }}>
            {[
              { label:'Ward', val:`Ward ${complaint.ward_number}${complaint.area ? ` · ${complaint.area}` : ''}` },
        ...
        <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'14px', padding:'16px 20px', marginBottom:'12px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div>
            <div style={{ fontSize:'14px', fontWeight:'700', color:'white', marginBottom:'2px' }}>
              {complaint.upvotes||0} people have this issue
            </div>
        ...
        <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'14px', padding:'20px 24px', marginBottom:'24px' }}>
          <div style={{ fontSize:'11px', fontWeight:'700', color:'rgba(255,255,255,0.3)', marginBottom:'20px', textTransform:'uppercase', letterSpacing:'0.8px' }}>
            Resolution timeline
          </div>
          {TIMELINE.map((step, i) => {
            ...
            return (
              <div key={step.key} style={{ display:'flex', gap:'14px' }}>
                ...
                <div style={{ paddingTop:'4px', paddingBottom: i < TIMELINE.length-1 ? '0' : '0' }}>
