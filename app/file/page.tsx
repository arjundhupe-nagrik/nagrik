...
function SuccessScreen({ complaintId, areaName, pinCoords, onReset, router, isLoggedIn }: any) {
    ...
    <main style={{ minHeight:'100vh', background:'#0D0D14', fontFamily:"'Inter',system-ui,sans-serif", display:'flex', alignItems:'center', justifyContent:'center', padding:'24px' }}>
      <div style={{ maxWidth:'440px', width:'100%' }}>
        <div style={{ textAlign:'center', marginBottom:'24px' }}>
          <div style={{ width:'72px', height:'72px', background:'rgba(61,170,110,0.15)', border:'2px solid rgba(61,170,110,0.4)', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 16px', fontSize:'32px' }}>✓</div>
          <div style={{ fontSize:'26px', fontWeight:'900', color:'white', marginBottom:'6px', letterSpacing:'-0.5px' }}>Complaint filed!</div>
          <div style={{ fontSize:'14px', color:'rgba(255,255,255,0.4)' }}>
            Filed for <strong style={{ color:'white' }}>{areaName}</strong>
            {pinCoords && <span style={{ color:'#3DAA6E' }}> · 📍 Location pinned</span>}
          </div>
        </div>

        <div style={{ background:'rgba(232,115,26,0.08)', border:'1px solid rgba(232,115,26,0.25)', borderRadius:'14px', padding:'20px', marginBottom:'16px' }}>
          <div style={{ fontSize:'11px', color:'rgba(255,255,255,0.35)', textTransform:'uppercase', letterSpacing:'0.8px', marginBottom:'8px' }}>Your complaint tracking ID</div>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:'12px' }}>
            <div style={{ fontSize:'28px', fontWeight:'900', color:'#E8731A', letterSpacing:'4px' }}>NMC-{complaintId}</div>
            <button onClick={copyId}
              style={{ background:copied?'rgba(61,170,110,0.15)':'rgba(255,255,255,0.08)', border:'1px solid '+(copied?'rgba(61,170,110,0.3)':'rgba(255,255,255,0.12)'), color:copied?'#3DAA6E':'rgba(255,255,255,0.5)', padding:'8px 14px', borderRadius:'8px', fontSize:'12px', fontWeight:'600', cursor:'pointer' }}>
              {copied?'Copied':'Copy'}
            </button>
          </div>
          <div style={{ fontSize:'11px', color:'rgba(255,255,255,0.25)', marginTop:'10px', lineHeight:'1.5' }}>
            {isLoggedIn ? 'Linked to your account. Track it in My Complaints.' : 'Save this ID to track your complaint later.'}
          </div>
        </div>
        ...
        <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'12px', padding:'16px', marginBottom:'16px' }}>
          <div style={{ fontSize:'11px', fontWeight:'700', color:'rgba(255,255,255,0.35)', textTransform:'uppercase', letterSpacing:'0.8px', marginBottom:'12px' }}>What happens next</div>
          {[
            { icon:'📬', text:'Your complaint is now public and visible to authorities' },
            { icon:'🔍', text:'Officials will review and assign it to the right department' },
            { icon:'🔔', text: isLoggedIn ? 'You will be notified when your complaint status changes' : 'Sign in to get notified when your complaint status changes' },
          ].map((s,i) => (
            <div key={i} style={{ display:'flex', alignItems:'flex-start', gap:'10px', marginBottom:i<2?'10px':'0' }}>
              ...
              <span style={{ fontSize:'13px', color:'rgba(255,255,255,0.5)', lineHeight:'1.5' }}>{s.text}</span>
            </div>
          ))}
        </div>
          ...
          {isLoggedIn ? (
            <>
              <button onClick={() => router.push('/my-complaints')}
                style={{ background:'#E8731A', color:'white', border:'none', padding:'14px', borderRadius:'10px', fontSize:'14px', fontWeight:'700', cursor:'pointer', width:'100%' }}>
                View my complaints →
              </button>
              <button onClick={() => router.push('/')}
                style={{ background:'rgba(255,255,255,0.05)', color:'rgba(255,255,255,0.6)', border:'1px solid rgba(255,255,255,0.1)', padding:'14px', borderRadius:'10px', fontSize:'14px', fontWeight:'600', cursor:'pointer', width:'100%' }}>
                Back to dashboard
              </button>
            </>
          ) : (
            <>
              ...
              <button onClick={() => router.push('/login')}
                style={{ background:'rgba(61,170,110,0.08)', color:'#3DAA6E', border:'1px solid rgba(61,170,110,0.25)', padding:'14px', borderRadius:'10px', fontSize:'14px', fontWeight:'700', cursor:'pointer', width:'100%' }}>
                🔐 Sign in to track your complaint
              </button>
            </>
          )}
          <button onClick={shareWhatsApp}
            style={{ background:'rgba(37,211,102,0.08)', color:'rgba(37,211,102,0.8)', border:'1px solid rgba(37,211,102,0.2)', padding:'12px', borderRadius:'10px', fontSize:'13px', fontWeight:'600', cursor:'pointer', width:'100%' }}>
            📲 Share on WhatsApp
          </button>
          <button onClick={onReset}
            style={{ background:'transparent', color:'rgba(255,255,255,0.3)', border:'1px solid rgba(255,255,255,0.08)', padding:'12px', borderRadius:'10px', fontSize:'13px', fontWeight:'600', cursor:'pointer', width:'100%' }}>
...
export default function FilePage() {
  ...
  useEffect(() => {
    ...
    const initMap = () => {
      ...
      const map = L.map(mapDivRef.current).setView([startLat, startLng], pinCoords?16:13)
      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution:'© OpenStreetMap' }).addTo(map)
      const icon = L.divIcon({ html:'<div style="background:#E8731A;width:20px;height:20px;border-radius:50%;border:3px solid white;box-shadow:0 2px 8px rgba(0,0,0,0.4)"></div>', iconSize:[20,20], iconAnchor:[10,10], className:'' })
      const locateIcon = L.divIcon({ html:'<div style="background:#185FA5;width:16px;height:16px;border-radius:50%;border:3px solid white;box-shadow:0 2px 8px rgba(0,0,0,0.5)"></div>', iconSize:[16,16], iconAnchor:[8,8], className:'' })
      if (pinCoords) markerRef.current = L.marker([pinCoords.lat, pinCoords.lng], { icon }).addTo(map).bindPopup('📍 Complaint location').openPopup()
      map.on('click', (e: any) => {
        const { lat, lng } = e.latlng
        if (markerRef.current) markerRef.current.remove()
        markerRef.current = L.marker([lat, lng], { icon }).addTo(map).bindPopup('<b>📍 Complaint location</b><br>'+lat.toFixed(5)+', '+lng.toFixed(5)).openPopup()
        ...
        onAdd: () => {
          ...
          L.DomEvent.on(btn, 'click', () => {
            ...
            navigator.geolocation.getCurrentPosition(
              ...
              () => { btn.innerHTML = '✗ Denied'; setTimeout(() => { btn.innerHTML = '📍 My location' }, 2000) },
  ...
  useEffect(() => {
    ...
    const L = (window as any).L
    if (!L) return
    fetch('https://nominatim.openstreetmap.org/search?q='+encodeURIComponent(form.area_name+' Nagpur India')+'&format=json&limit=1', { headers:{ 'User-Agent':'nagrik-civic-app' } })
      .then(r=>r.json()).then(data => { if (data?.[0]) mapRef.current.setView([parseFloat(data[0].lat), parseFloat(data[0].lon)], 15) }).catch(()=>{})
  }, [form.ward_number])
  ...
  async function detectLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        try {
          const res = await fetch('https://nominatim.openstreetmap.org/reverse?lat='+latitude+'&lon='+longitude+'&format=json&accept-language=en', { headers:{ 'User-Agent':'nagrik-civic-app' } })
          const data = await res.json()
          ...
          for (const c of candidates) {
            matched = areas.find(a => a.area_name.toLowerCase().replace(/\s/g,'').includes(c.toLowerCase().replace(/\s/g,'')) || c.toLowerCase().replace(/\s/g,'').includes(a.area_name.toLowerCase().replace(/\s/g,'')))
            if (matched) break
          }
          if (matched) { selectArea(matched); setDetectMsg('✓ Location detected: '+matched.area_name) }
          else { setDetectMsg('Location found — please select your area from the list'); setShowDropdown(true); setAreaSearch('') }
        } catch { setDetectMsg('Coordinates saved — please select your area manually'); setShowDropdown(true) }
        setDetecting(false)
      },
      (err) => {
        setDetecting(false)
        const msgs: Record<number,string> = { 1:'Permission denied — please select area manually', 2:'Location unavailable — please select area manually', 3:'Timed out — please select area manually' }
        setDetectMsg(msgs[err.code]||'Location error'); setShowDropdown(true)
      },
  ...
  async function submit() {
    ...
    const { data, error } = await supabase.from('complaints').insert([{
      ...
      status: 'open',
      upvotes: 0,
      source: currentUser ? 'registered' : 'anonymous',
  ...
  if (submitted) return (
    <SuccessScreen complaintId={complaintId} areaName={form.area_name} pinCoords={pinCoords} onReset={resetForm} router={router} isLoggedIn={!!currentUser} />
  )
  ...
  return (
    <main style={{ minHeight:'100vh', background:'#0D0D14', fontFamily:"'Inter',system-ui,sans-serif" }}>
      ...
      <nav style={{ borderBottom:'1px solid rgba(255,255,255,0.06)', padding:'0 32px', height:'58px', display:'flex', alignItems:'center', justifyContent:'space-between', background:'rgba(13,13,20,0.85)', backdropFilter:'blur(12px)' }}>
        <a href={currentUser?'/':'/login'} style={{ fontSize:'20px', fontWeight:'800', color:'#E8731A', textDecoration:'none' }}>nagrik</a>
        <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
          {currentUser ? (
            <>
              <a href="/" style={{ color:'rgba(255,255,255,0.4)', fontSize:'13px', textDecoration:'none', padding:'6px 14px' }}>Dashboard</a>
              <a href="/my-complaints" style={{ color:'rgba(255,255,255,0.4)', fontSize:'13px', textDecoration:'none', padding:'6px 14px' }}>My complaints</a>
            </>
          ) : (
            <a href="/login" style={{ color:'rgba(255,255,255,0.5)', fontSize:'13px', textDecoration:'none', border:'1px solid rgba(255,255,255,0.1)', padding:'6px 14px', borderRadius:'8px' }}>Sign in</a>
          )}
        </div>
      </nav>

      <div style={{ maxWidth:'560px', margin:'0 auto', padding:'40px 24px' }}>
        <h1 style={{ fontSize:'28px', fontWeight:'900', color:'white', marginBottom:'6px', letterSpacing:'-0.5px' }}>What is the issue?</h1>
        ...
        <div style={{ marginBottom:'16px' }}>
          ...
          <input value={form.title} onChange={e=>setField('title',e.target.value)} placeholder="e.g. Pothole on main road"
            style={{ background:'rgba(255,255,255,0.05)', border:'1px solid '+(form.title.length>0?'rgba(232,115,26,0.4)':'rgba(255,255,255,0.08)'), color:'white', borderRadius:'10px', padding:'14px', fontSize:'15px', width:'100%', outline:'none' }} />
        </div>

        <div style={{ marginBottom:'20px' }}>
          <label style={{ fontSize:'11px', fontWeight:'700', color:'rgba(255,255,255,0.4)', display:'block', marginBottom:'8px', textTransform:'uppercase', letterSpacing:'0.8px' }}>Description</label>
          <textarea value={form.description} onChange={e=>setField('description',e.target.value)} placeholder="How long? How bad? How many people affected?" rows={3}
          ...
          <div style={{ background:'rgba(232,115,26,0.1)', border:'1px solid rgba(232,115,26,0.3)', borderRadius:'10px', padding:'14px 16px', marginBottom:'20px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:'12px' }}>
            ...
            <div style={{ display:'flex', gap:'8px', flexShrink:0 }}>
              <button onClick={() => { setForm(f=>({...f,category:aiSuggestion!})); setAiSuggestion(null) }}
                style={{ background:'#E8731A', color:'white', border:'none', borderRadius:'8px', padding:'8px 16px', fontSize:'13px', fontWeight:'700', cursor:'pointer' }}>Use this</button>
              <button onClick={() => setAiSuggestion(null)}
                style={{ background:'transparent', color:'rgba(255,255,255,0.3)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'8px', padding:'8px 12px', fontSize:'13px', cursor:'pointer' }}>X</button>
            </div>
          </div>
        )}
        ...
        <div style={{ marginBottom:'24px' }}>
          ...
          {detectMsg && (
            <div style={{ fontSize:'12px', color:detectMsg.startsWith('✓')?'#3DAA6E':'rgba(255,255,255,0.45)', marginBottom:'8px', padding:'8px 12px', background:'rgba(255,255,255,0.04)', borderRadius:'8px' }}>{detectMsg}</div>
          )}
          ...
          <div style={{ position:'relative' }}>
            <input value={areaSearch} onChange={e=>{ setAreaSearch(e.target.value); setForm(f=>({...f,ward_number:'',area_name:''})); setShowDropdown(true) }}
            ...
            {showDropdown && !form.ward_number && (
              <div style={{ position:'absolute', top:'calc(100% + 4px)', left:0, right:0, background:'#13131F', border:'1px solid rgba(255,255,255,0.12)', borderRadius:'10px', maxHeight:'240px', overflowY:'auto', zIndex:100, boxShadow:'0 8px 32px rgba(0,0,0,0.4)' }}>
                {areas.length===0 && <div style={{ padding:'14px 16px', fontSize:'13px', color:'rgba(255,255,255,0.3)' }}>Loading...</div>}
                {areas.length>0 && filteredAreas.length===0 && <div style={{ padding:'14px 16px', fontSize:'13px', color:'rgba(255,255,255,0.3)' }}>No areas found</div>}
                {filteredAreas.slice(0,30).map(area => (
                  <div key={area.ward_number} onMouseDown={()=>selectArea(area)}
        ...
        <div style={{ marginBottom:'32px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'8px' }}>
            <label style={{ fontSize:'11px', fontWeight:'700', color:'rgba(255,255,255,0.4)', textTransform:'uppercase', letterSpacing:'0.8px' }}>Pin exact location (optional)</label>
          ...
          {pinCoords && (
            <div style={{ fontSize:'12px', color:'#3DAA6E', display:'flex', alignItems:'center', gap:'6px' }}>
              ...
              <button onClick={()=>{ setPinCoords(null); setForm(f=>({...f,latitude:'',longitude:''})); if(markerRef.current){markerRef.current.remove();markerRef.current=null} }}
                style={{ background:'transparent', border:'none', color:'rgba(255,255,255,0.3)', cursor:'pointer', fontSize:'11px' }}>remove</button>
            </div>
          )}
        </div>
